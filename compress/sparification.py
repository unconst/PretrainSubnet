...
def decompress_topr(values, masks, original_shape):
    masks = unpack_uint8_tensor(masks)
    x = torch.zeros(original_shape, dtype=values.dtype, device=values.device)
    x.view(-1)[masks.view(-1).bool()] = values
    return x
...
def decompress_topk2(values, masks, original_shape):
    masks = unpack_uint8_tensor(masks)
    x = torch.zeros(original_shape, dtype=values.dtype, device=values.device)
    x.view(-1)[masks.view(-1).bool()] = values
    return x
